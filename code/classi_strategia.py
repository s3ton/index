...
class BaseCryptoStrategy(ABC):
    ...
    # --- NUOVI METODI PLOT SPOSTATI DALLA CLASSE FIGLIA ---
    ...
    def import_data(self, assets: list, timeframe: str = "1y", save_historical: bool = True, historical_dir: str = "../data/historical"):
        ...
        print("Download tasso di cambio EUR/USD per conversione prezzi...")
        eurusd_series = None
        try:
            eurusd_df = yf.Ticker('EURUSD=X').history(period=timeframe)
            if not eurusd_df.empty:
                eurusd_series = eurusd_df['Close']
                try:
                    eurusd_series.index = pd.to_datetime(eurusd_series.index).tz_convert('UTC').normalize()
                except TypeError:
                    eurusd_series.index = pd.to_datetime(eurusd_series.index).normalize()
            else:
                print("Avviso: Dati EUR/USD non trovati, i prezzi rimarranno in USD.")
        except Exception as e:
            print(f"Errore download EUR/USD: {e}. I prezzi rimarranno in USD.")
        ...
        for ticker_symbol in assets:
            try:
                ...
                if eurusd_series is not None:
                    try:
                        temp_idx = pd.to_datetime(df_raw.index).tz_convert('UTC').normalize()
                    except TypeError:
                        temp_idx = pd.to_datetime(df_raw.index).normalize()
                        
                    aligned_rates = eurusd_series.reindex(temp_idx, method='ffill').bfill().values
                    df_raw['Close'] = df_raw['Close'] / aligned_rates
                    df_raw['Volume'] = df_raw['Volume'] / aligned_rates
        ...
        if self.benchmark_ticker in self.data_dict:
            ...
            try:
                ...
                if not df_bench_raw.empty:
                    if eurusd_series is not None:
                        try:
                            temp_idx = pd.to_datetime(df_bench_raw.index).tz_convert('UTC').normalize()
                        except TypeError:
                            temp_idx = pd.to_datetime(df_bench_raw.index).normalize()
                            
                        aligned_rates = eurusd_series.reindex(temp_idx, method='ffill').bfill().values
                        df_bench_raw['Close'] = df_bench_raw['Close'] / aligned_rates

                    df_bench_final = df_bench_raw[['Close']].copy()
                    df_bench_final.columns = ['Prezzo di Chiusura']
                    self.benchmark_df = df_bench_final
                    print(f"Benchmark {self.benchmark_ticker} completato con successo (prezzi in EUR).\n")
                else:
        ...
        if btc_ticker in df_prices.columns:
            ...
            try:
                if self.benchmark_df is not None and not self.benchmark_df.empty:
                    btc_initial_price = float(self.benchmark_df['Prezzo di Chiusura'].iloc[0])
                else:
                    raise ValueError("Benchmark non disponibile per settare il capitale iniziale.")
            except Exception as e:
                raise ValueError(f"Impossibile recuperare il prezzo base al {first_date}: {e}")
                
        if pd.isna(btc_initial_price) or btc_initial_price <= 0:
            raise ValueError(f"Il prezzo di riferimento al primo giorno non è valido: {btc_initial_price}")
            
        portfolio_value = float(btc_initial_price)
        self.portfolio_value = portfolio_value
        print(f"Valore iniziale del portafoglio impostato al prezzo di {btc_ticker} in data {first_date.strftime('%Y-%m-%d')}: {portfolio_value:.2f} EUR")
    ...
    def run_strategy(self, rebalance_freq_days: int = 180, min_mktcap: float = 1_000_000_000, btc_ticker: str = 'BTC-USD', **kwargs):
        ...
        if btc_ticker in df_prices.columns:
            ...
            if self.benchmark_df is not None and not self.benchmark_df.empty:
                 ...
                 print("Impossibile recuperare il prezzo iniziale. Uso 10000.0 come fallback.")
...
class GlobalMarketAnalytics(BaseCryptoStrategy):
    ...
    def generate_analytics_charts(self, 
        ...
        fig_vol.update_layout(
            margin=dict(l=40, r=40, t=40, b=40), plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
            hovermode="x unified", yaxis=dict(title="Volume 24h (€)", tickformat=",.0f", ticksuffix=" €")
        )
